...
async function initDB() {
  ...
  try {
    ...
    if (adminEmail && adminPassword) {
      ...
      const adminId2 = existing.length > 0 ? existing[0].id : (await conn.execute("SELECT id FROM organizations WHERE email = ?", [adminEmail]))[0]?.[0]?.id;
      if (adminId2) {
        ...
        if (pressRows.length === 0) {
          ...
          for (const post of seedPosts) {
            await conn.execute(
              ...
              [post.title, post.content, "press_release", adminId2, null]
            );
          }
